...
import { BehaviorSubject, Observable, Subject, Subscription, from, fromEvent, interval, merge, of, zip } from 'rxjs';
import { bufferCount, debounceTime, delay, distinctUntilChanged, filter, map, skip, switchMap, switchMapTo, take, takeUntil, tap, throttleTime } from 'rxjs/operators';
import { DraggableDirective, DraggableModule, Keys, ResizeSensorComponent, ResizeSensorModule, anyChanged, guid, hasObservers, isChanged } from '@progress/kendo-angular-common';
...
class DataResultIterator {
    constructor(source, skip$$1 = 0, groupFooters = false) {
        this.source = source;
        this.skip = skip$$1;
 ...
 * @hidden
 */
class CellKeydownEvent extends PreventableEvent {
...
class NavigationCursor {
    ...
    offsetRow(offset) {
        ...
        if (this.metadata && this.metadata.isVirtual) {
            ...
            const nextIndex = Math.max(0, Math.min(this.activeRow + offset, maxIndex));
            this.virtualRow = nextIndex;
            let nextRow = this.model.findRowByIndex(nextIndex);
            if (nextRow) {
                const cell = nextRow.cells.find(c => c.colIndex === this.virtualCol);
                if (cell && cell.parent) {
                    nextColIndex = cell.parent.colIndex;
                }
            }
        ...
        switch (args.keyCode) {
            ...
            case Keys.PageDown:
                if (this.metadata.isVirtual && this.viewport) {
                    ...
                    const nextItemIndex = this.viewport.lastItemIndex + this.meta.headerRows + 1;
            ...
            case Keys.PageUp:
                if (this.metadata.isVirtual && this.viewport) {
                    ...
                    const nextItemIndex = Math.max(0, this.viewport.firstItemIndex - viewportSize - 1);
...
class NavigationMetadata {
    constructor(dataRows, headerRows, isVirtual, hasPager, gridElement) {
...
class ScrollerService {
    ...
    create(rowHeightService, skip$$1, take$$1, total) {
        ...
        const subject = new BehaviorSubject(new ScrollAction(this.rowHeightService.offset(skip$$1)));
...
class DataBindingDirective {
    ...
    applyState({ skip: skip$$1, take: take$$1, sort, group, filter: filter$$1 }) {
        this.skip = skip$$1;
...
class PagerPrevButtonsComponent extends PagerElementComponent {
    ...
    onChanges({ total, skip: skip$$1, pageSize }) {
        this.total = total;
        this.skip = skip$$1;
...
const skipButtons = and(not(isDeleteButton), not(isSortIcon), not(isFocusable), not(matchesNodeName("label")));
 ...
 * @hidden
 */
const hasFilter = (settings, column) => settings.filter && column.field && column.filterable;
/**
 * @hidden
 */
const hasSort = (settings, column) => settings.sort && column.field && column.sortable;
/**
 * @hidden
 */
 ...
 * @hidden
 */
const cloneFilters = ({ filters, logic }) => ({
    filters: filters.map(x => isCompositeFilterDescriptor(x) ? cloneFilters(x) : cloneFilter(x)),
    logic
});
/**
 * @hidden
 ...
 * @hidden
 */
const slice = (groups, skip$$1, take$$1, includeFooters = false) => {
    if (!isPresent(take$$1)) {
        return groups;
    }
    const result = [];
        ...
        if (!skipHeader || itemCount) {
            ...
            if (take$$1) {
                if (hasGroups(groupItems)) {
                    const children = slice(groupItems, skip$$1, take$$1, includeFooters);
                    items.push(...children);
                    take$$1 -= count(children, includeFooters);
                }
                else {
                    items.push(...groupItems.slice(skip$$1, Math.min(skip$$1 + take$$1, groupItems.length)));
                    take$$1 -= items.length;
                }
...
class PDFComponent extends PDFExportComponent {
    ...
    changePage(skip$$1, _take, callback, columns) {
        this.ngZone.run(() => {
            ...
            this.component.notifyPageChange('pdf', { skip: skip$$1, take: _take });
        });
    }
...
class ExcelComponent {
    constructor(excelService, localization) {
        this.localization = localization;
        /**
