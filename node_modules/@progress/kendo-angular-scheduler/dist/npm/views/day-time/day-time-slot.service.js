...
function initHorizontalSlots(slots, items, rowHeight, eventHeight) {
    ...
    sorted.forEach(function (event) { return slots
        ...
        event.rowIndex = utils_1.findRowIndex(value.events, event);
    ...
    Object.keys(slotItems).forEach(function (key) {
        ...
        for (var idx = 0; idx < events.length; idx++) {
            ...
            if (event_2) {
                ...
                event_2.rect.width += rect.width + constants_1.BORDER_WIDTH;
            }
        }
    });
    items.forEach(function (item) {
        item.rect.width -= constants_1.BORDER_WIDTH;
        item.reflow();
    });
}
