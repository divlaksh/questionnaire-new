...
var BaseView = /** @class */ (function () {
    function BaseView(viewContext, viewState, intl, slotService, zone, renderer, element, pdfService, localization) {
        ...
        this.changes = new rxjs_1.BehaviorSubject(null);
    ...
    BaseView.prototype.ngOnInit = function () {
        ...
        this.subs.add(rxjs_1.combineLatest(this.viewContext.items, this.viewState.dateRange)
        ...
        this.subs.add(rxjs_1.combineLatest(this.items, this.changes, this.localization.changes).pipe(operators_1.switchMap(function () { return _this.onStable(); }))
    ...
    BaseView.prototype.dragResourcesByIndex = function (index) {
        ...
        for (var idx = 0; idx < allResources.length; idx++) {
            if (!result[idx]) {
                result[idx] = utils_1.resourceItemByValue(this.dragging.task.event, allResources[idx]);
            }
        }
        return result;
    };
    ...
    BaseView.prototype.onStable = function () {
        return this.zone.onStable.asObservable().pipe(operators_1.take(1));
    };
    ...
    BaseView.prototype.bindEvents = function () {
        ...
        this.zone.runOutsideAngular(function () {
            if (_this.times) {
                _this.subs.add(rxjs_1.merge(rxjs_1.fromEvent(_this.times.nativeElement, 'mousewheel'), rxjs_1.fromEvent(_this.times.nativeElement, 'DOMMouseScroll')).pipe(operators_1.filter(function (event) { return !event.ctrlKey; }), operators_1.tap(dom_queries_1.preventLockedScroll(contentElement)), operators_1.map(dom_queries_1.wheelDeltaY))
                    .subscribe(function (x) { return contentElement.scrollTop -= x; }));
            }
            _this.subs.add(rxjs_1.merge(util_1.fromClick(contentElement), rxjs_1.fromEvent(contentElement, 'contextmenu'))
                .subscribe(function (e) { return _this.onClick(e); }));
            _this.subs.add(util_1.fromDoubleClick(contentElement)
    ...
    BaseView.prototype.dragResourceColor = function (task, slotResources) {
        if (this.groupedResources.length) {
            ...
            return util_1.getField(slotResources[index], this.groupedResources[0].colorField);
        }
    ...
    BaseView.prototype.resourceValues = function (task, currentResources) {
        ...
        for (var idx = 0; idx < currentResources.length; idx++) {
            ...
            if (Array.isArray(resourceItem)) { // not grouped multiple resource
                value = util_1.getField(task.event, resource.field);
            }
            else {
                value = util_1.getField(currentResources[idx], resource.valueField);
    ...
    BaseView.prototype.emitTaskEvent = function (e, eventType) {
        ...
        if (targetTask) {
            ...
            if (isSingle && dom_queries_1.closestInScope(e.target, function (node) { return dom_queries_1.hasClasses(node, 'k-event-delete'); }, targetTask.target)) {
                e.preventDefault();
    ...
    BaseView.propDecorators = {
        еventTemplate: [{ type: core_1.Input }],
